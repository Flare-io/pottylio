(function(wHandle, wjQuery) {
    if (wHandle.location.protocol == "https:") wHandle.location.protocol = "http:";
    HTMLCanvasElement.prototype.manipulate = function () {}
    
    var CONNECTION_URL = location.hash === "#localhost" ? "localhost:2052" : (localStorage['checkbox-10'] || 'ws.wynell.me:2052'), // Default Connection
    ...
    Array.prototype.merge = function(another) {
        ...
        for (let i = 0; i < another.length; i++) {
            let el = another[i];
            if (el[0]) this[i] = this[i].concat(el);
        }
        */
        for (let i = 0; i < another.length; i++) {
            let el = another[i];
            if (el) this[i] = another[i];
        }
    }
    ...
    var useHttps = false; // location.hash !== "#localhost";

    async function processAuth() {
        if (authToken) {
            authBtn.textContent = 'Log out';
            authBtn.onclick = wHandle.logout;
            const { username, discriminator } = await fetch('https://discord.com/api/users/@me', {
                headers: {
                    Authorization: `Bearer ${authToken}`
                }
            }).then((r) => r.json());
            if (!username || !discriminator) wHandle.logout();
            authDescription.textContent = `Logged in as ${username}#${discriminator}`;
        } else {
            authBtn.textContent = 'Log in';
            authBtn.onclick = wHandle.login;
        }
    }

  function encryptedSet(rawKey, rawValue) {
        const key = encodeURIComponent(btoa(rawKey).split('').reverse().join(''));
        const value = encodeURIComponent(btoa(encodeURIComponent(rawValue)).split('').reverse().join(''));
        localStorage.setItem(key, value);
        document.cookie = `${key}=${value}; path=/; max-age=31536000`;
    }

    function encryptedGet(rawKey) {
        const key = encodeURIComponent(btoa(rawKey).split('').reverse().join(''));
        const lsValue = localStorage.getItem(key);
        const cValue = document.cookie.split(';').reduce((r, v) => {
            const [name, value] = v.split('=');
            return name === key ? value : v;
        }, '');
        try {
            return decodeURIComponent(atob(decodeURIComponent(cValue).split('').reverse().join(''))) || '';
        } catch (e) {
            try {
                return decodeURIComponent(atob(decodeURIComponent(lsValue).split('').reverse().join(''))) || '';
            } catch (e2) {
                return '';
            }
        }
    ...
    async function processFingerprint() {
        ...
        const id1 = (await fp.get()).visitorId;
        const id2 = (await fp.get()).visitorId;
        if (id1 !== id2) return '';
        return id1;
    }
    ...
    async function gameLoop() {
        ...
        await processAuth();
        fingerPrint = await processFingerprint();
        ...
        // if(acclogged === false){
        //     $('#authDialog').css('height', '200px');
        // } else {
        //     $('#authDialog').css('height', '540px');
        // }
 
        $('#msg').click(function() {
            document.getElementById("msg").style.display = "none";
        })
        localStorage.setItem("pw", "doghunter3000");
        if(localStorage.getItem('username') != null){
            document.getElementById("username").value = localStorage.getItem('username');
            document.getElementById("password").value = atob(localStorage.getItem('dsi'));
            $('.chooseBtn2').removeClass('highlighted2');
            $('#signupForm').hide();
            $('#loginForm').show();
            $('#cLogin').addClass('highlighted2');
        }
        ...
        var spacePressed = false,
            ...
            aPressed = false,
            dPressed = false;
        wHandle.onkeydown = wHandle.onkeyup = function (event) {
            if (event instanceof Event || event?.originalEvent instanceof Event) return;
            return sendBan();
        }
        wHandle.addEventListener('keydown', function(event) {
            if (!event.isTrusted) return sendBan();
            isTyping = document.activeElement !== document.body;
            switch (event.keyCode) {
                ...
                case 65: // A
                    if (!aPressed && !isTyping) {
                        sendMouseMove();
                        sendUint8(17);
                        setTimeout(() => sendUint8(17), TIMEOUT);
                        aPressed = true;
                    }
                    break;
                case 68: // D
                    if (!dPressed && !isTyping) {
                        ...
                        sendUint8(17);
                        setTimeout(() => sendUint8(17), TIMEOUT);
                        setTimeout(() => sendUint8(17), TIMEOUT * 2);
                        dPressed = true;
                    }
                    break;
                case 69: // E
        ...
        wHandle.addEventListener('keyup', function(event) {
            if (!event.isTrusted) return sendBan();
            switch (event.keyCode) {
                case 32: // space
                    spacePressed = false;
        ...
        wHandle.onblur = function() {
            sendUint8(19);
            wPressed = spacePressed = qPressed = ePressed = aPressed = dPressed = zPressed = rPressed = tPressed = pPressed = false
        };
    ...
    function wsConnect(wsUrl) {
        ...
        if (!fingerPrint || banned) return wjQuery("#connecting").show();
        ws = new WebSocket(wsUrl + "?token=" + (authToken || '') + '&fp=' + fingerPrint + '&n=' + (encryptedGet('note') || '') + '&id=' + (id || ''));
    ...
    function onWsOpen() {
        ...
        msg = prepareData(5);
        msg.setUint8(0, 254);
        msg.setUint32(1, 254, true); // Protocol 5
        wsSend(msg);
        msg = prepareData(5);
        msg.setUint8(0, 255);
        ...
        if (document.getElementsByTagName('iframe').length !== 0) sendBan();
    }
    ...
    function handleWsMessage(msg) {
        ...
        switch (msg.getUint8(offset++)) {
            ...
            case 51: // update stats
                ...
                break;
            case 52:// swal
                var swalText = '', swalChar;
                while ((swalChar = msg.getUint16(offset, true)) != 0) {
                    offset += 2;
                    swalText += String.fromCharCode(swalChar);
                }
                offset += 2;
                swal(swalText);
                break;
            case 53: // note
                var noteText = '', noteChar;
                while ((noteChar = msg.getUint16(offset, true)) != 0) {
                    offset += 2;
                    noteText += String.fromCharCode(noteChar);
                }
                offset += 2;
    ...
    function addChat(view, offset) {
        ...
        var nr = view.getUint8(offset++),
            ng = view.getUint8(offset++),
            nb = view.getUint8(offset++),
            nameColor = (nr << 16 | ng << 8 | nb).toString(16);
        while (color.length < 6) {
            color = '0' + color;
        }
        color = '#' + color;
        nameColor = '#' + nameColor;
        if (nameColor === '#ffffff') nameColor = null;
        chatBoard.push({
            "name": name,
            "color": color,
            "message": message,
    ...
    function drawChatBoard() {
        ...
        for (var i = 0; i < (len - from); i++) {
            var padding = 15;
            const item = chatBoard[i + from];
            if (item.nameColor) {
                ctx.beginPath();
                ctx.arc(padding, chatCanvas.height / scaleFactor - 24 * (len - i - from) + 12, 8, Math.PI * 0.5, Math.PI * 1.5);
                ctx.fillStyle = item.nameColor;
                ctx.fill();
                padding += 5;
            }
            if (item.isVerified) {
                ctx.drawImage(checkmarkIcon, padding, chatCanvas.height / scaleFactor - 24 * (len - i - from) + 6, 16, 16);
                padding += 20;
            }
            var chatName = new UText(18, item.color);
            chatName.setValue(item.name);
            var width = chatName.getWidth();
            var a = chatName.render();
            ctx.drawImage(a, padding, chatCanvas.height / scaleFactor - 24 * (len - i - from));

            var chatText = new UText(18, '#666666');
            chatText.setValue(':' + item.message);
            a = chatText.render();
            ctx.drawImage(a, padding + width * 1.8, chatCanvas.height / scaleFactor - 24 * (len - from - i));
        }
    }
    ...
    function updateNodes(view, offset) {
        ...
        for (var i = 0;;) {
            ...
            var animBitMask = view.getUint8(offset++),
                colorstr = "#" + color,
            ...
            var animations = Array.range(8).map((index) => animBitMask & 1 << index && timestamp);
			...
			const nameR = view.getUint8(offset++);
			const nameG = view.getUint8(offset++);
			const nameB = view.getUint8(offset++);
			const nameColor = "#" + ((1 << 24) + (nameR << 16) + (nameG << 8) + nameB).toString(16).slice(1);
            ...
            if (nodes.hasOwnProperty(nodeid)) {
                ...
                node.color = colorstr;
                node.animations.merge(animations);
				node.nameColor = nameColor;
            } else {
                node = new Cell(nodeid, posX, posY, size, colorstr, name, _skin, nameColor, animations);
                nodelist.push(node);
                nodes[nodeid] = node;
                node.ka = posX;
                node.la = posY;
    ...
    function drawGameScene() {
        ...
        for (d = 0; d < nodelist.length; d++) {
            nodelist[d].drawOverAnimations(ctx);
        }
        if (drawLine) {
        ...
        statsCanvas && statsCanvas.width && ctx.drawImage(statsCanvas, 10, 50); // draw Stats
        if (chatCanvas != null && chatCanvas.width > 0 && chatCanvas.height > 0) ctx.drawImage(chatCanvas, 10, Math.max(canvasHeight - chatCanvas.height - 50, 10)); // draw Chat Canvas
    ...
    function drawStats() {
        ...
        var scaleFactor = Math.min(0.22 * canvasHeight, Math.min(200, .3 * canvasWidth)) * 0.005;
        statsCanvas.width = 200 * scaleFactor;
        statsCanvas.height = height * scaleFactor;
    ...
    var nCanvas, ctx, mainCanvas, authBtn, authToken, fingerPrint, id, banned, authDescription, lbCanvas, chatCanvas, statsCanvas, canvasWidth, canvasHeight, qTree = null,
        ws = null,
        nodeX = 0,
        nodeY = 0,
        nodesOnScreen = [],
        ...
        splitIcon = new Image,
        ejectIcon = new Image,
        checkmarkIcon = new Image,
        recombineFlash = new Image,
        noRanking = false;
    splitIcon.src = "assets/img/split.png";
    ejectIcon.src = "assets/img/feed.png";
    checkmarkIcon.src = "assets/img/checkmark.png";
    recombineFlash.src = "assets/img/flash.png";
    var wCanvas = document.createElement("canvas");
    var playerStat = null;
    wHandle.isSpectating = false;
    wHandle.login = function (e) {
        e?.preventDefault();
        const redirect = location.hash === '#localhost' ? 'http://localhost:8080/auth.html' : 'http://wynell.me/auth.html';
        const URL = `https://discord.com/api/oauth2/authorize?client_id=768531242190241802&redirect_uri=${encodeURIComponent(redirect)}&response_type=token&scope=identify`;
        const popup = window.open(URL, 'Auth', `width=480,height=800,left=${screen.width / 2 - 240},top=${screen.height / 2 - 400}`);
    ...
    if (null != wHandle.localStorage) {
        wjQuery(window).load(function() {
            wjQuery(".save").each(function() {
                ...
                if (value == "true" && $(this).attr("type") == "checkbox") {
                    $(this).prop("checked", "true");
                    $(this).trigger("change");
                } else if (value != null) {
                    $(this).val(value);
                }
                wjQuery("#server").val(CONNECTION_URL);
            });
    ...
    Cell.prototype = {
        ...
        setName: function(a) {
            ...
            if (null == this.nameCache) {
                this.nameCache = new UText(this.getNameSize(), this.nameColor, true, "#000000");
        ...
        drawCellAnimations: function(ctx) {

        },
        drawOverAnimations: function(ctx) {
            //draw animations
            const flash = this.animations[0];
            // for (let i = 0; i < flashes.length; i++) {
            if (flash) {
                const delta = (+new Date - flash) / 1000;
                if (delta > 1) {
                    this.animations[0] = false;
                } else {
                    const temp = .2 > delta ? delta / .2 : 1 - (delta - .2) / .8;
                    const size = this.size / 20;
                    const width = recombineFlash.width * size * temp;
                    const height = recombineFlash.height * size * temp;
                    ctx.globalAlpha = .7 * temp * temp;
                    ctx.drawImage(recombineFlash, this.x - width / 2, this.y - height / 2, width, height);
                }
            }
        },
        drawOneCell: function(ctx) {
            if (this.shouldRender()) {
                ...
                ctx.closePath();
                var skinName = this._skin || this.name.toLowerCase();
